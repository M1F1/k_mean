...
class TestGaussianMixtureModel(unittest.TestCase):
    ...
    def test_init_values(self):
        ...
        test_cluster_probability_matrix = np.array([[0.6, 0.4],
                                                    [0.7, 0.3]])
        test_input_data = np.array([[-10, 0],
                                    [0, 10],
                                    [-10, 1]])
        test_cluster_means = np.array([[-10, 0],
                                       [0, 10]])

        # gmm = GaussianMixtureModel(cluster_number=2, data=test_input_data)
        # res4 = gmm._compute_clusters_covariances(test_cluster_probability_matrix,
        #                                          test_cluster_means,
        #                                          test_input_data)
        # print(res4)
        gmm.fit()
